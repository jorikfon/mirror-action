...
if [[ "${GIT_SSH_PRIVATE_KEY}" != "" ]]; then
    ...
    if [[ "${GIT_SSH_KNOWN_HOSTS}" != "" ]]; then
      ...
      git config --global core.sshCommand "ssh -i ~/.ssh/id_rsa -o IdentitiesOnly=yes -p 2222 -o UserKnownHostsFile=~/.ssh/known_hosts"
    else
      if [[ "${GIT_SSH_NO_VERIFY_HOST}" != "true" ]]; then
        ...
        git config --global core.sshCommand "ssh -i ~/.ssh/id_rsa -o IdentitiesOnly=yes -p 2222 -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no"
      fi
    fi
else
